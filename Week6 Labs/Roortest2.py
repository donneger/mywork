...
# Newtons method uses successive iterations of the formula: root = 0.5 * (X + (N / X)) where X is any guess which can be assumed to be N or 1.
# A desired accuracy level the user can input is also included 
# Source https://www.geeksforgeeks.org/find-root-of-a-number-using-newtons-method/



# Define the function for calculation, the number to be rooted and the accuracy level are passed in from the program. 
...
print ("The number input was: {}. The Square Root of the number is {}".format(number, sqroot(number, accuracy)))
